...
# Five restarts + boot time start trips trips the default start rate limit.
...
at_exit() {
    ...
    systemctl stop \
              dbus-reconnect-recover.service \
              dbus-reconnect-stop.service \
              dbus-reconnect-exit.service \
              dbus-reconnect-drop.service \
              dbus-reconnect-notify.service \
              dbus-reconnect-notify-target.service \
...
restart_broker() {
    systemctl kill --signal=KILL dbus.service
    timeout 90 bash -c '
        until busctl --system status >/dev/null 2>&1; do
            systemctl reset-failed dbus.service dbus.socket 2>/dev/null
            systemctl start dbus.socket 2>/dev/null
            sleep 0.2
        done'
}
...
systemd-run \
    ...
    -p "ExecReload=/usr/bin/touch $reloaded" \
    /usr/bin/true
...
systemd-run \
    ...
    -p Restart=no \
    -p NotifyAccess=main \
    -p PropagatesReloadTo="$target" \
    "$holder" "$name" "$marker" "$release" "$notify"
restart_broker
poll "$unit" SubState running-revalidating
touch "$notify"
