...
int bus_machine_method_get_addresses(sd_bus_message *message, void *userdata, sd_bus_error *error) {
        ...
        n = machine_get_addresses(m, &addresses, &reterr_errno);
        if (n == -ENONET)
                return sd_bus_error_setf(error, BUS_ERROR_NO_PRIVATE_NETWORKING, "Machine %s does not use private networking", m->name);
        if (n == -ENOEXEC)
                return sd_bus_error_set_errnof(error, reterr_errno, "Failed to fork(): %m");
        if (n == -ECHILD)
                return sd_bus_error_set_errnof(error, reterr_errno, "Failed to wait for child: %m");
        if (n == -ESHUTDOWN)
